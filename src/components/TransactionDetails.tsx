            ...
            {transaction.operations.map((op, index) => (
              <div key={index} className="bg-gray-50 p-4 rounded-lg">
                ...
                {op.type === 'invoke_host_function' && (
                  <div className="space-y-1 text-sm">
                    <p><span className="text-gray-600">Contract interaction by:</span> 
                       <span className="font-mono text-blue-600 ml-1">{op.source_account || transaction.sourceAccount}</span></p>
                    {transaction.sorobanOperations?.find(sop => sop.contractId === op.source_account) && (
                      <div className="mt-2 p-2 bg-purple-50 rounded">
                        <p className="text-xs text-purple-700 font-medium">
                          Function: {transaction.sorobanOperations.find(sop => sop.contractId === op.source_account)?.functionName}
                        </p>
                      </div>
                    )}
                  </div>
