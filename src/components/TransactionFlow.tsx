...
import { OperationNode, TransactionEffect, SorobanOperation } from '../types/stellar';
...
function TransactionFlowInner({ nodes, edges, effects = [], sorobanOperations = [] }: TransactionFlowProps) {
  ...
  }, [nodes, executionStep, layoutMode]);

  const adjustedEdges = showConnections ? edges.map((edge, index) => {
